...
public class CustomerCLI {
  ...
  private void listCustomers() {
    ...
    if (list.isEmpty()) {
      System.out.println(Color.YELLOW + "⚠ No customers found" + Color.RESET);
    } else {
      System.out.println(Color.CYAN + "┌──────────┬──────────────────────┬─────────────────┬────────────┐");
      System.out.println("│   ID     │        Name          │    Contact      │ Purchases  │");
      System.out.println("├──────────┼──────────────────────┼─────────────────┼────────────┤" + Color.RESET);

      for (Customer c : list) {
        int purchaseCount = c.getSalesHistory() != null ? c.getSalesHistory().size() : 0;
        System.out.printf("│ %-8s │ %-20s │ %-15s │ %-10d │\n",
            c.getId(),
            c.getName(),
            c.getContact(),
            purchaseCount);
  ...
  private void viewCustomerSalesHistory() {
    ...
    if (history == null || history.isEmpty()) {
      System.out.println(Color.YELLOW + "⚠ " + "No sales found for this customer." + Color.RESET);
      return;
    }
